...
def _build_counterfactual_context(model, outputs, batch, epoch, pair):
    ...
    # Local ordinal E replaces the earlier masked-video E objective entirely.
    # Masked forwards restore every RNG stream and clone parent buffers; these
    # unused diagnostics do not advance training randomness. Wrong-query fields
    # and the actual T direction objective are still computed above.
    if model.options.a_local_evidence and not getattr(
        model, "retain_legacy_counterfactual_diagnostics", False
    ):
        context["report"].update(
            legacy_masked_e_path_disabled=True,
            legacy_masked_e_diagnostics_computed=False,
        )
        return context
    if not bits & 1:
        return context
