"""Candidate evidence, support, and start/end transition scoring heads."""
...
class _SupportFieldScoreHeads(Round1FieldScoreHeads):
    ...
    @staticmethod
    def _support_diagnostics(h, token_valid, rows=32):
        """Exact diagnostic values without a full [B,L,L,H] autograd graph."""
        with torch.no_grad():
            h = h.float().masked_fill(~token_valid[..., None], 0)
            count = token_valid.float()[..., None]
            prefix = lambda x: torch.nn.functional.pad(x.cumsum(1), (0, 0, 1, 0))
            ...
            for offset in range(0, length, rows):
                start = index[offset:offset + rows, None]
                n = pc[:, end] - pc[:, start]
                mean = (ph[:, end] - ph[:, start]) / n.clamp_min(1)
                variance = (
                    (p2[:, end] - p2[:, start]) / n.clamp_min(1) - mean.square()
                ...
                left, right = (start - 2).clamp_min(0), (end + 2).clamp_max(length)
                outside_n = pc[:, start] - pc[:, left] + pc[:, right] - pc[:, end]
                variances.append(variance.mean(-1))
                neighbors.append(outside_n.squeeze(-1))
            return torch.cat(variances, 1), torch.cat(neighbors, 1)
    ...
    def add_support_structure(self, field, state, video_padding_mask):
        ...
        diagnostics_only = (
            roles.shape[1] > 128 and not self.support_context
            and not self.support_dispersion
        )
        if diagnostics_only:
            with torch.no_grad():
                variance_mean, neighbors = self._support_diagnostics(
                    self.support.encode(roles[:, :, 1, :]), mask
                )
            contrast = variance = None
        else:
            contrast, variance, neighbors = self.support_statistics(
                self.support.encode(roles[:, :, 1, :]), mask
            )
            variance_mean = variance.mean(-1)
        ...
        return SupportStructureOutput(
            **values,
            ...
            support_variance_mean=variance_mean.masked_fill(~field.valid, 0),
        )
...
class _LegacyFieldScoreHeads(_SupportFieldScoreHeads):
    ...
    def raw_from_state(self, state, valid, carrier, video_padding_mask=None):
        ...
        if roles.shape[1] > 128 and not self.evidence_pool and not self.evidence_context:
            # Only the valid-token neighbor count survives when both branches
            # are disabled; it is independent of encoded evidence features.
            count = mask.float()[..., None]
            pc = torch.nn.functional.pad(count.cumsum(1), (0, 0, 1, 0))
            length = mask.shape[1]
            index = torch.arange(length, device=mask.device)
            start, end = index[:, None], index[None, :] + 1
            left, right = (start - 2).clamp_min(0), (end + 2).clamp_max(length)
            neighbors = (pc[:, start] - pc[:, left] + pc[:, right] - pc[:, end]).squeeze(-1)
            pooled = contrast = None
        else:
...
class ThreeFieldScoreHeads(_LegacyFieldScoreHeads):
    """Evidence, support, and transition readouts on the candidate span grid.
    
    Parameter construction and primitive operations retain checkpoint-compatible
    names and numerical ordering."""
