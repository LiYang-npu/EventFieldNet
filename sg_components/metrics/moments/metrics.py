...
from sg_components.metrics.moments.avg_precision import (
    compute_average_precision_detection_wrapper,
)
from sg_components.metrics.moments.misc import (
    DataContainerType,
    get_data_by_range,
    prepare_data,
)
from sg_components.metrics.moments.utils import (
...
class MRAveragePrecision(Metric):
    ...
    def update(
        self, submissions: DataContainerType, targets: DataContainerType
    ) -> None:  # noqa: WPS210
        ...
        ranged_submissions, ranged_targets = get_data_by_range(
            submissions, targets, window_length
        )
        ...
        compute_ap_from_triple = partial(
            compute_average_precision_detection_wrapper, tiou_thresholds=self.iou_trhds
        )
    ...
    def compute(self) -> Dict[str, Tensor]:
        ...
        ap_per_iou = (
            ap_per_query_per_iou.mean(0) * 100
...
class MRRecallAt1(Metric):
    ...
    @staticmethod
    def prepare_data(
        submissions: DataContainerType, targets: DataContainerType
    ) -> Tuple[Any, Any]:  # noqa: WPS602
        ...
        """
        pred_qid2window = {
            submission["qid"]: submission["pred_relevant_windows"][0][:2]
    ...
    def update(
        ...
        if ranged_targets:
            pred_qid2window, gt_qid2window = self.prepare_data(
                ranged_submissions, ranged_targets
            )

            # get window arrays
            qids = list(pred_qid2window.keys())
            pred_windows = np.array([pred_qid2window[qid] for qid in qids]).astype(
                float
            )
    ...
    def compute(self) -> Dict[str, Tensor]:
        ...
        Returns:
            Dict[str, Tensor]: The Average Presion Score somputed per each IoU threshold.
        """
        if not self.computed_ious:
            zero_tensor = torch.tensor(0, dtype=torch.float32)
            iou_thd2recall_at_one: Dict[str, Tensor] = {
                str(float(f"{thd:.2f}")): zero_tensor
                for thd in self.iou_trhds  # noqa: WPS221
            }
