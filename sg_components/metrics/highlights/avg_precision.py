...
def get_ap(y_true: Tensor, y_predict: Tensor, interpolate: bool = True) -> Tensor:
    ...
    """
    assert y_true.size(0) == y_predict.size(0), (
    ...
    if torch.unique(y_true).size(0) == 1:
        ...
        assert sorted(set(y_true.numpy())) == [0, 1], (
...
def compute_ap_from_tuple(
    input_tuple: Tuple[int, int, Tensor, Tensor],
    ...
    if y_true.size(0) < y_predict.size(0):
        ...
        y_predict_new = torch.zeros_like(
            y_true, device=y_predict.device, dtype=torch.float32
        )
