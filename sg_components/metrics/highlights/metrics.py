...
from sg_components.metrics.highlights.avg_precision import compute_hl_ap
from sg_components.metrics.highlights.hit1 import compute_hl_hit1, mk_saliency_scores
...
class HIT1Coef(Metric):
    ...
    saliency_thresholds: Dict[str, int] = {
        "Binary": 1,
        "Fair": 2,
        "Good": 3,
        "VeryGood": 4,
    ...
    def update(
        self, submissions: List[Dict[str, Any]], targets: List[Dict[str, Any]]
    ) -> None:  # noqa: WPS221
        ...
        # prepare inputs
        qid_preds = {
            submission["qid"]: submission["pred_saliency_scores"]
        ...
        qid_saliency_thresholded = {
            qid: (saliency >= threshold) for qid, saliency in saliency_gts.items()
        }
...
class AveragePrecision(Metric):
    ...
    def update(
        ...
        # prepare inputs
        qid_preds = {
            submission["qid"]: submission["pred_saliency_scores"].cpu()
            for submission in submissions
        }
        saliency_gts = {target["qid"]: mk_saliency_scores(target) for target in targets}
...
class MAPTop5ForAnnotators(Metric):
    ...
    def update(
        ...
        qid_targets = {
            target["qid"]: torch.tensor(target["saliency_scores"]) for target in targets
        }
