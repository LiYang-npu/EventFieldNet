...
from sg_components.utils.tensor_utils import pad_sequences_1d
...
def custom_collate(batch: List[Dict[str, Any]]) -> Tuple[MetaTyping, DataTyping]:  # noqa: WPS231
    ...
    for key in model_inputs_keys:
        if key == "span_labels":
            batched_data[key] = [
                {"spans": sample["model_inputs"][key]} for sample in batch
            ]
            continue

        if key in {"saliency_pos_labels", "saliency_neg_labels"}:
            batched_data[key] = torch.LongTensor(
...
def move_inputs_to_device(  # noqa: WPS234
    ...
    if "saliency_all_labels" in batched_model_inputs:
        ...
        targets["saliency_all_labels"] = targets["saliency_all_labels"].to(
            device, non_blocking=non_blocking
        )
        targets["relevant_clips"] = targets["saliency_all_labels"].to(
            device, non_blocking=non_blocking
        )
