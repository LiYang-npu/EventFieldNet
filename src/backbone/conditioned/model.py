"""C3-compatible model shell with explicit, versioned extension points."""
...
from backbone.model import Stage55Model, Stage55Output
...
@dataclass
class C3ExperimentOutput(Stage55Output):
    extension_output: Optional[C3ExtensionOutput] = None


class C3ExperimentModel(Stage55Model):
    """The strongest C3 carrier plus one additive full-span extension.

    IdentityExtension is mathematically equivalent to Stage55 C3. A custom
    extension may add logits and losses on the same span grid; it cannot move
    coordinates or consume old prediction scores through this API version.
    """
    ...
    @staticmethod
    def _base_fields(output: Stage55Output) -> dict[str, Any]:
        return {
            field.name: getattr(output, field.name) for field in fields(Stage55Output)
        }

    @staticmethod
    def _video_pad(inputs: Any, output: Stage55Output) -> Tensor:
    ...
    def forward(
        ...
        return C3ExperimentOutput(**values, extension_output=extension_output)
    ...
    def diagnostics(
        self, outputs: C3ExperimentOutput, batch: Any
