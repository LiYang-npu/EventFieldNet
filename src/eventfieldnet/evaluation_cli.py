...
def _install_import_roots(
    raw: Mapping[str, Any],
    *,
    root: Path,
    project_root: Path,
) -> tuple[Path, ...]:
    for path in reversed(_path_list(root, project_root)):
        text = str(path)
        if text not in sys.path:
            sys.path.insert(0, text)
    try:
        child = importlib.import_module("field_core.child")
        helper = getattr(child, "install_import_roots")
    except (ImportError, ModuleNotFoundError) as exc:
        raise Round31EvaluatorError(
            "reviewed field_core.child.install_import_roots is unavailable"
        ) from exc
    if not callable(helper):
        raise Round31EvaluatorError("install_import_roots is not callable")
    try:
        _bridge_root, paths = helper(raw, remote_root=str(root), strict=True)
    except Exception as exc:
        raise Round31EvaluatorError(
            "reviewed repository import-root resolution failed"
        ) from exc
...
def _prepare_model_and_bundle(
    ...
    *,
    ...
    try:
        ...
        shared = importlib.import_module("training.runner")
