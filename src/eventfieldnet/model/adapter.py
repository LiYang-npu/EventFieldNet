"""Round31 adapter around the reviewed reusable parent model."""
...
# fixed in Round31Options.from_values, so (like y8/y9) a new named
...
class TriFieldRound31Model(TriFieldBaseModel):
    """Parent carrier plus fixed Round31 E/S/T structure and losses."""

    def __init__(
        self,
        parent_model: nn.Module,
        *,
        options: Round31Options | None = None,
        selector_hidden_dim: int = 64,
        selector_lr: float = 1.0e-4,
        parent_factory: str = DEFAULT_PARENT_FACTORY,
    ) -> None:
        options = options or Round31Options.from_values()
        ...
        with torch.random.fork_rng(devices=[]):
            round31_selector = Round31FieldScoreHeads(
                input_dim=512,
...
def build_trifield_round31_model(
    ...
    **kwargs: Any,
    ...
    options = Round31Options.from_values(
        carrier_mode=carrier_mode,
        linear_calibration=linear_calibration,
        pairwise_interactions=pairwise_interactions,
        variant=variant,
...
Round31BaseModel = TriFieldRound31Model
