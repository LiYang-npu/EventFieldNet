...
class Round1FieldScoreHeads(TriFieldScoreHeads):
    ...
    def raw_from_state(
        ...
        nonlinear_input = torch.cat(
            (support_span.float(), width.unsqueeze(-1)),
            dim=-1,
        )
        # This residual is the only additional S path in the nonlinear slot.
        delta = self.support_nonlinear(nonlinear_input).squeeze(-1)
