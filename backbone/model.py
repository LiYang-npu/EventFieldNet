"""Shared multimodal span backbone and candidate/boundary prediction heads.

The model preserves the two-second clip coordinate system and exposes token,
query, interval-score and endpoint tensors to the field encoders."""
...
class SharedSpanHead(nn.Module):
    ...
    def _span_scores(self, tokens, query, valid_token, starts, ends, width):
        """The original full-grid MLP, isolated for activation recomputation."""
    ...
    def forward(
        ...
        if length > 128 and torch.is_grad_enabled():
            from torch.utils.checkpoint import checkpoint

            # All metadata is an explicit input: no mutable mask or parent
            # forward state is captured by the recomputation function.
            score = checkpoint(
                self._span_scores, tokens, query, valid_token, starts, ends, width,
                use_reentrant=False,
            )
        else:
            score = self._span_scores(tokens, query, valid_token, starts, ends, width)
...
@dataclass
class SpanBackboneOutput:
    """Span scores, boundary predictions, and shared video/query features."""
    span_logits: Tensor
    span_probs: Tensor
...
class SpanBackboneModel(EventFieldNetBoundaryPreserving):
    """Shared multimodal backbone with interval and boundary prediction heads."""
    ...
    @staticmethod
    def _iou(outputs: SpanBackboneOutput, batch: Any) -> Tensor:
    ...
    def compute_loss(
        self, outputs: SpanBackboneOutput, batch: Any, teacher_outputs: Any, epoch: int
    ) -> LossResult:
    ...
    def diagnostics(self, outputs: SpanBackboneOutput, batch: Any) -> Mapping[str, Tensor]:
        del batch
        valid = outputs.span_valid_mask
        return {
