...
def span_readout(value: Tensor, readout, width: Tensor | None = None) -> Tensor:
    """Recompute large span features during backward instead of retaining them.

    The long-sequence path executes the same full-grid operations, without
    changing candidate membership, FP32 arithmetic, or readout parameters.
    Non-reentrant checkpointing also supports the model's autograd.grad calls.
    Short QV sequences keep their original execution path.
    """
    def project(tokens):
        spans = span_mean(tokens)
        if width is not None:
            spans = torch.cat((spans.float(), width.unsqueeze(-1)), dim=-1)
        return readout(spans)

    if value.shape[1] > 128 and torch.is_grad_enabled():
        from torch.utils.checkpoint import checkpoint

        return checkpoint(project, value, use_reentrant=False)
...
class TriFieldScoreHeads(nn.Module):
    ...
    def raw_from_state(
        ...
        raw_evidence = span_readout(evidence_token, self.evidence.readout)
        raw_support = span_readout(support_token, self.support.readout) + self.support_width(
            width.unsqueeze(-1)
        ).squeeze(-1)
